...
module Sprockets
  ...
  if Sprockets::Less::Utils.version_of_sprockets >= 3
    ...
    # anything. For Sprockets 2.x , although the file and the module name
    # exist, they can't be used because it will give errors about undefined
    # methods, because this is included only on Sprockets::Base and in order
    # to use them we would have to subclass it and define methods to expire
    # cache and other methods for registration , which are not needed since
    # Sprockets already knows about that using the environment instead
    # internally
